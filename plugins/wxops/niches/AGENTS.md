...
| 段 | 必填 | 说明 |
|---|---|---|
| `recommendations` | 否 | 静态运营建议；写出后三子字段必填（issue #60） |
| `scoring` | 否 | 赛道级打分覆写命名空间；当前仅 `interaction_thresholds`，内部字段全可选（issue #74）。内置 `ai-tools` / `_generic` **故意不写**，平台默认即按其实测标定 |
...
- `scoring.interaction_thresholds` 无本赛道实测支撑不要覆写；非法值在 `load_niche` 阶段 fail-fast。
