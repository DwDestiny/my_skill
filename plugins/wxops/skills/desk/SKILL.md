...
用户问"我现在该干嘛"时：跑 desk，按「下一步」列给出具体命令，**一次只推一步**，别把全流程倒给用户。建议规则（P1 版，基于时间戳）：

- 从未登录 / 登录记录过老 → 先 `wxops login --account <slug>`
- 已登录、无数据或数据 > 7 天 → `wxops analyze --account <slug>`
- 数据较新 → 可直接看报告 / 看板，或进入内容工位

登录列目前按最近登录时间推断（真探测在后续版本升级为主动健康检查）；如果用户反馈"明明登录过却拉数失败"，让其重新 `login` 即可。
