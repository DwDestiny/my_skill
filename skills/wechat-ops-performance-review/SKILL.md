...
> **使用边界**:本 skill 仅用于抓取使用者**本人拥有管理权限**的公众号后台数据,服务于自有账号运营复盘;须遵守微信公众平台服务协议,主动限频,不伪造指纹、不绕过平台机制,不抓取无授权的第三方账号。
...
先看效果、不想登录抓取:`scripts/wxops analyze --demo --data-only`(把 skill 内 `fixtures/` 原始输入复制到工作区,只产数据、不碰 pnpm,**无需登录、无需 Node/pnpm**,零额外依赖)。想连本地看板一起跑,去掉 `--data-only` 即可,但需本机有 **Node >= 18 + pnpm** 构建看板。
